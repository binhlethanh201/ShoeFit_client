...
const Settings = () => {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          if (user.preferences) {
            if (user.preferences.shoe_size_unit)
              setShoeSizeStd(user.preferences.shoe_size_unit);
          }
  ...
  const SettingItem = ({ icon, title, description, children, danger }) => (
    ...
    >
      <div className="d-flex align-items-center">
        <div
          ...
          style={{
            ...
            backgroundColor: danger
              ? "rgba(220, 53, 69, 0.1)"
              : "var(--bg-section)",
            color: danger ? "#dc3545" : "var(--primary)",
          }}
        >
        ...
        <div>
          <h6
            className={`mb-0 fw-semibold ${danger ? "text-danger" : ""}`}
            style={{ color: danger ? "" : "var(--text-heading)" }}
          >
            {title}
          </h6>
          {description && (
  ...
  return (
    <div className="container py-5" style={{ maxWidth: "720px" }}>
      <h3 className="fw-bold mb-4" style={{ color: "var(--text-heading)" }}>
        {t("settings.title")}
      </h3>
      <div
          ...
          <div className="d-flex gap-2">
            {["light", "dark", "coffee"].map((mode) => (
              <button
                ...
                style={{
                  backgroundColor:
                    theme === mode ? "var(--primary)" : "transparent",
                  borderColor:
                    theme === mode ? "var(--primary)" : "var(--border-color)",
